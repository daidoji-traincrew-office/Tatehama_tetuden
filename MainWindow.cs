...
namespace RailwayPhone
{
    ...
    public class MainWindow : Window
    {
        ...
        private UdpVoiceManager _voiceManager; // 音声通話用
        private const string SERVER_IP = "127.0.0.1"; // サーバーIP
        private const int SERVER_PORT = 8888;

        // --- デバイス設定 (エラー修正: これらが必須です) ---
        ...
        private string _connectedTargetNumber; // 通話相手の番号
        ...
        // テキスト部品
        private TextBlock _statusNameText;     // 宛先名

        // ★重要: ここが TextBox でないとエラーになります
        private TextBox _inputNumberBox;       // 番号入力欄

        private TextBlock _incomingNameText;   // 着信名
        private TextBlock _incomingNumberText; // 着信番号
        private TextBlock _outgoingNameText;   // 発信名
        private TextBlock _outgoingNumberText; // 発信番号

        private TextBlock _talkingStatusText;  // 状態表示
        private TextBlock _talkingNameText;    // 通話相手名
        private TextBlock _talkingTimerText;   // タイマー
        ...
        public MainWindow(PhoneBookEntry station)
        {
            ...
            _voiceManager = new UdpVoiceManager(); // 音声マネージャー初期化
            ...
            Closing += (s, e) => {
                if (_voiceManager != null) _voiceManager.Dispose();
        ...
        private async void ConnectToServer()
        {
            ...
            // スレッドエラー対策: Dispatcher.Invokeを使用
            Dispatcher.Invoke(() =>
            {
                Title = $"模擬鉄 指令電話端末 - [{_currentStation.Name}]";
                if (success)
        ...
        private void OnMessageReceived(string json)
        {
            Dispatcher.Invoke(() =>
            {
                try
                {
                    ...
                    {
                        case "INCOMING":
                            _targetIpAddress = fromIp;
                            _targetUdpPort = fromPort;
        ...
        private void GoToTalkingScreen(bool isOutgoing)
        {
            ...
            if (_currentOutputDevice != null) int.TryParse(_currentOutputDevice.ID, out outDev);

            _voiceManager.StartTransmission(_targetIpAddress, _targetUdpPort, inDev, outDev);
        }
        ...
        private UIElement CreateIncomingView()
        {
            ...
            rejectBtn.Click += (s, e) => EndCall(false);
        ...
        private UIElement CreateOutgoingView()
        {
            ...
            var cancelBtn = new Button { Content = "取 消", Width = 200, Height = 60, Background = _dangerColor, Foreground = Brushes.White, FontSize = 18, FontWeight = FontWeights.Bold, Cursor = System.Windows.Input.Cursors.Hand };
            var cStyle = new Style(typeof(Border)); cStyle.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30))); cancelBtn.Resources.Add(typeof(Border), cStyle);
            cancelBtn.Click += (s, e) => EndCall(true);
        ...
        private UIElement CreateTalkingView()
        {
            ...
            var hs = new Style(typeof(Border)); hs.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30))); _holdBtn.Resources.Add(typeof(Border), hs);
            ...
            var es = new Style(typeof(Border)); es.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30))); endBtn.Resources.Add(typeof(Border), es);
        ...
        private Button CreateDialButton(string t)
        {
            ...
            var s = new Style(typeof(Border)); s.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30)));
