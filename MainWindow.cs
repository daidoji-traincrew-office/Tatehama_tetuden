...
using CommunityToolkit.WinUI.Notifications; // 通知機能用
using System.IO; // ファイルチェック用
...
namespace RailwayPhone
{
    public class MainWindow : Window
    {
        ...
        private UIElement _viewKeypad;   // 1. 待機・発信画面
        private UIElement _viewIncoming; // 2. 着信画面
        private UIElement _viewTalking;  // 3. 通話画面
        ...
        // --- カラー定義 ---
        private readonly Brush _primaryColor = new SolidColorBrush(Color.FromRgb(0, 120, 215)); // Winブルー
        ...
        private readonly Brush _bgColor = new SolidColorBrush(Color.FromRgb(240, 244, 248));    // 背景色

        // コンストラクタ
        public MainWindow(PhoneBookEntry station)
        {
            // 安全対策: もしデータが空ならダミーを入れる
            if (station == null)
            {
            ...
            // ★重要: 音響マネージャーをここで初期化する
            // もしこれが抜けていると NullReferenceException になります
            ...
            Closing += (s, e) => {
                ...
                ToastNotificationManagerCompat.Uninstall(); // 通知の後始末
            };
        }
        ...
        private void InitializeComponents()
        {
            ...
            // ★重要: ここですべてのビューを作って追加しておく（Null防止）
            ...
            _viewIncoming.Visibility = Visibility.Collapsed; // 最初は隠す
            _rightPanelContainer.Children.Add(_viewIncoming);

            // 3. 通話画面 (Talking)
            _viewTalking = CreateTalkingView();
            _viewTalking.Visibility = Visibility.Collapsed; // 最初は隠す
        ...
        private UIElement CreateIncomingView()
        {
            ...
            var answerBtn = new Button { Content = "📞 応答", Height = 60, Background = _acceptColor, Foreground = Brushes.White, FontSize = 18, FontWeight = FontWeights.Bold, Cursor = System.Windows.Input.Cursors.Hand };
            var aStyle = new Style(typeof(Border)); aStyle.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30))); answerBtn.Resources.Add(typeof(Border), aStyle);
            answerBtn.Click += (s, e) => AnswerCall();
            Grid.SetColumn(answerBtn, 0); btnGrid.Children.Add(answerBtn);

            var rejectBtn = new Button { Content = "切断", Height = 60, Background = _dangerColor, Foreground = Brushes.White, FontSize = 18, FontWeight = FontWeights.Bold, Cursor = System.Windows.Input.Cursors.Hand };
            var rStyle = new Style(typeof(Border)); rStyle.Setters.Add(new Setter(Border.CornerRadiusProperty, new CornerRadius(30))); rejectBtn.Resources.Add(typeof(Border), rStyle);
        ...
        private UIElement CreateTalkingView()
        {
            ...
            _talkingTimerText = new TextBlock { Text = "00:00", FontSize = 48, FontFamily = new FontFamily("Consolas"), Foreground = Brushes.DarkSlateGray, HorizontalAlignment = HorizontalAlignment.Center, Margin = new Thickness(0, 0, 0, 50) };
            panel.Children.Add(_talkingTimerText);

            var endBtn = new Button { Content = "終 話", Width = 200, Height = 60, Background = _dangerColor, Foreground = Brushes.White, FontSize = 18, FontWeight = FontWeights.Bold, Cursor = System.Windows.Input.Cursors.Hand };
        ...
        private void SimulateIncomingCall()
        {
            ...
            {
                // 1. 各変数のNullチェック (診断)
                if (_currentStation == null) throw new NullReferenceException("原因: '_currentStation' (自局情報) が null です。");
                if (PhoneBook.Entries == null) throw new NullReferenceException("原因: 'PhoneBook.Entries' が null です。");
                if (_soundManager == null) throw new NullReferenceException("原因: '_soundManager' が null です。コンストラクタで初期化されていません。");
                if (_viewIncoming == null) throw new NullReferenceException("原因: '_viewIncoming' (着信画面) が null です。");
                if (_incomingNameText == null) throw new NullReferenceException("原因: '_incomingNameText' が null です。");

                // 2. 相手を選ぶ
                var others = PhoneBook.Entries.Where(x => x.Number != _currentStation.Number).ToList();
                if (others.Count == 0)
                {
                    MessageBox.Show("電話帳に自分以外の連絡先がないため、着信テストができません。");
                    return;
                }
                var caller = others[new Random().Next(others.Count)];

                // 3. 画面切り替え
                if (_viewKeypad != null) _viewKeypad.Visibility = Visibility.Collapsed;
                if (_viewTalking != null) _viewTalking.Visibility = Visibility.Collapsed;
                ...
                // 4. 音再生 (Sounds/yobi2.wav)
                if (_currentOutputDevice != null)
                {
                    _soundManager.SetOutputDevice(_currentOutputDevice.ID);
                }

                // ファイルが存在するか一応チェック
                string soundPath = "Sounds/yobi2.wav"; // ★あなたの環境に合わせてSounds(複数形)にしました
                if (!File.Exists(soundPath))
                {
                    // ファイルがない場合はエラーにせずログだけ出す
                    System.Diagnostics.Debug.WriteLine($"音声ファイルが見つかりません: {soundPath}");
                }
                else
                {
                    // ファイルがあれば再生
                    _soundManager.Play(soundPath, loop: true);
                }
            ...
            {
                // ここでエラーの原因をメッセージボックスで表示
                MessageBox.Show($"テスト実行中にエラーが発生しました。\n\n【詳細】\n{ex.Message}\n\n{ex.StackTrace}", "エラー診断", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        }
        ...
        private void StartOutgoingCall()
        {
            ...
            _soundManager.Play("Sounds/yobi1.wav", loop: true); // Sounds(複数形)に変更
        ...
        {
            _soundManager.Stop();
            _soundManager.Play("Sounds/tori.wav"); // Sounds(複数形)に変更
        ...
        {
            _callTimer?.Stop();
            _soundManager.Play("Sounds/oki.wav"); // Sounds(複数形)に変更
